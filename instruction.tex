...
\begin{verbatim}
python src/preprocessing/data_pipeline.py download \
...
python src/preprocessing/data_pipeline.py inspect \
    --data-dir /path/to/data/raw \
    --report-dir /path/to/data/reports
...
python src/preprocessing/make_balanced_split.py \
    --input-split /path/to/data/splits/patient_split.json \
    --output-split /path/to/data/splits/patient_split_balanced.json
\end{verbatim}
...
python train.py \
    --processed-dir /path/to/data/processed \
    --split-file /path/to/data/splits/patient_split_balanced.json \
...
Training also writes \texttt{saved\_models/metrics.json}, which stores the saved hyperparameters and final validation/test metrics.
...
\begin{itemize}[nosep]
    \item \texttt{test\_metrics.json} -- Deterministic test metrics, temperature-scaled test metrics, and MC Dropout summary metrics.
...
MC Dropout error-detection AUROC/AUPRC for predictive entropy, standard deviation, and mutual information are printed to the terminal during evaluation.
